...
**Mode Prédiction** : les jours sans aucun log entre le 1er log et hier sont estimés à la moyenne arrondie de J par jour actif (`computeGuessed()`, entrées `estimated:true` en mémoire seulement — jamais écrites dans Firebase). Affichage calendrier : variant hachuré "Calendar cell / Guessed" (node 36-461, fond clair strié, jour `#1d1d1d`, conso bleue "4j"). Les estimations comptent dans les stats conso, le graphique J 12 mois et l'historique du jour (lignes bleues « J n - estimé », heures fixes 9h/13h/17h/21h, non éditables/supprimables, numérotation et intervalles partagés avec les entrées réelles). Les jours d'achat sans log gardent leur case jaune mais affichent leur estimation (« 4j » bleu). Le mode Normal n'affiche que les données réelles. Titre du hero forcé sur 2 lignes (`<br>`, cf. bug historique n°5) pour ne pas wrapper en 3 lignes sur mobile quand le bouton affiche « Prédiction ».
